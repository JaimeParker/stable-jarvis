...
### Files Modified
- `stable_jarvis/report_generator/converter.py`: Added `ImageMetadata`, `extract_images_with_metadata()`, `save_image_manifest()`
- `stable_jarvis/report_generator/__init__.py`: Export `ImageMetadata`
- `stable_jarvis/__init__.py`: Export `ImageMetadata`
...
### Files Added
- `stable_jarvis/report_generator/__init__.py`
- `stable_jarvis/report_generator/converter.py`
- `stable_jarvis/report_generator/py.typed`
- `tests/test_pdf_converter.py`

### Usage
```python
from stable_jarvis import PDFConverter, ConversionOptions
...
**IMPORTANT**: All agents and scripts MUST read Zotero credentials (library_id, api_key, library_type) from `config/zotero.json` only. Never hardcode these values in scripts.
...
_zotero_config_path = _script_dir.parent / "config" / "zotero.json"
...
Restructured the project as a modern Python package with proper `pyproject.toml` configuration, enabling `pip install -e .` and clean imports via `from stable_jarvis import ...`.

## Changes

### Package Structure
- Created `src/stable_jarvis/` directory (renamed from `src/auto-annotation/` to fix Python import compatibility — hyphens are invalid in Python identifiers)
...
- Refactored `config.py` with flexible configuration loading:
  ...
  2. **Priority 2**: Config file search (`./zotero.json` → `./config/zotero.json` → `~/.config/stable-jarvis/zotero.json`)
...
# Import the package
python -c "from stable_jarvis import ZoteroAnnotator; print('OK')"
```

## Verification

```
$ pip show stable-jarvis
Name: stable-jarvis
Version: 0.1.0
...
Restructured the package from `src/stable_jarvis/` to a modular layout at `stable_jarvis/annotation/`, preparing for future submodules.
...
from stable_jarvis import ZoteroAnnotator, annotate

# Submodule import (explicit)
from stable_jarvis.annotation import ZoteroAnnotator, annotate
```

### pyproject.toml Update
- Changed `[tool.setuptools.packages.find]` from `where = ["src"]` to `where = ["."]` with `include = ["stable_jarvis*"]`
