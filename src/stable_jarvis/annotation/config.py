...
2. Config file search: ./zotero.json → ./config/zotero.json → ~/.config/stable-jarvis/zotero.json
"""
...
def _get_config_search_paths() -> List[Path]:
    ...
    2. ./config/zotero.json (relative to cwd)
    3. ~/.config/stable-jarvis/zotero.json (user config dir)
    """
    cwd = Path.cwd()
    home = Path.home()
    ...
    return [
        cwd / "zotero.json",
        cwd / "config" / "zotero.json",
        home / ".config" / "stable-jarvis" / "zotero.json",
    ]
