...
## Integration with stable-jarvis
...
### Enhanced Retrieval: Tool Mapping
...
- `report.md` — the full deep-reading report
- `good-question-bridge.json` — structured bridge file (generated by paper-deep-reader Phase 8)
- `zotero_kb.json` — Zotero knowledge base search results
- `meta.json` — paper metadata

**Consumption**:
1. If `good-question-bridge.json` exists, load it directly. It contains pre-extracted stated limitations, implied weaknesses, future directions, and personal insights — these map to the "Has a candidate idea" starting state in the workflow (Step 1 diagnosis)
2. If only `report.md` exists, read the "局限性与未来方向" (Limitations & Future Directions) and "个人评价与启发" (Personal Insights) sections — these become the candidate ideas
3. The `zotero_kb.json` and `obsidian_kb.json` (if present) provide pre-built Domain Brief material — do not rebuild from scratch. Use these to identify what related work already exists and what gaps are genuine
4. The `meta.json` provides the source paper's title, authors, arxiv ID — use this to establish what has already been done (the deep-reader paper itself), so candidate questions are positioned beyond it

**Workflow shortcut**: When bridge files are detected, the workflow compresses:
- Step 0 (Information Gate): context is already rich → skip to Step 1
- Step 1 (Diagnose): auto-set to "Has a candidate idea" with multiple specific starting points
- Step 2 (Build Context): pre-filled from meta.json + KB files + bridge file
...
- Good Question Cards → `00 Inbox/Research Questions/{paper_title_short} - Research Question.md`
- Use `[[wikilinks]]` to link back to:
  - The deep-reader report (if applicable): `[[{paper_title} Deep Reading Report]]`
  - Related KB notes identified during analysis
  - Relevant Zotero entries
- Frontmatter includes: tags, source paper, date, zotero_key (if applicable)

**outputs/ directory** (for process archival and debugging):
- Full analysis record → `outputs/good-question/{item_key or date}/`
  - `domain-brief.md` — compact Domain Brief with evidence ledger
  - `candidates.md` — all candidate questions with scores and lens annotations
  - `shortlist.md` — ranked shortlist with repair/rejection notes
  - `editor-desk-gate.md` — desk-reject checks for top candidates
  - `good-question-cards.md` — final Good Question Cards
- If bridged from paper-deep-reader, use the same `item_key` for cross-referencing
...
See the upstream repository for worked examples: <https://github.com/Rimagination/good-question/tree/main/examples>
